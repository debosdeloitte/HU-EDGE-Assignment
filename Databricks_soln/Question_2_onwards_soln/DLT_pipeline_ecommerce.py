...
# MAGIC %md
# MAGIC %md
# MAGIC ## 2. Delta Live Tables (DLT) pipeline : Streaming ETL & Quality Enforcement 
# MAGIC
# MAGIC
...
# # ---- Bronze: raw events — CSV read ----
# @dlt.table(comment="Raw events from CSV")
# def bronze_events():
...
# # ---- Bronze: raw users — CSV read ----
# @dlt.table(comment="Raw users from CSV")
# def bronze_users():
#     return (spark.read.csv(f"{BASE}/users.csv", header=True, multiLine=True, nullValue="null", inferSchema=True)
#             .withColumn("user_first_touch_timestamp", (F.col("user_first_touch_timestamp")/1e6).cast("timestamp")))

# # ---- Silver: flattened sales items (matches Part1 Cell 7) ----
# @dlt.table(comment="Flattened sales items")
# def silver_sales_items():
...
# # ---- Silver: flattened event items (matches Part1 Cell 7) ----
# @dlt.table(comment="Flattened event items")
# def silver_event_items():
#     return (dlt.read("bronze_events")
#             .select("user_id", "event_name", "traffic_source", "event_timestamp", F.explode("items").alias("i"))
#             .select("user_id", "event_name", "traffic_source", "event_timestamp", "i.*"))

# # ---- Silver: cleansed events with quality rules ----
# @dlt.table(comment="Cleansed events")
# @dlt.expect_or_drop("valid_user", "user_id IS NOT NULL")
# @dlt.expect("non_negative_ts", "event_timestamp IS NOT NULL")
# def silver_events():
#     return dlt.read("bronze_events")

# # ---- Silver: daily revenue per product ----
# @dlt.table(comment="Daily revenue per product")
# def silver_daily_product_revenue():
#     e = dlt.read("silver_events")
#     return (e.select(F.to_date("event_timestamp").alias("day"), F.explode("items").alias("i"))
#              .groupBy("day", "i.item_id")
#              .agg(F.sum("i.item_revenue_in_usd").alias("revenue")))
