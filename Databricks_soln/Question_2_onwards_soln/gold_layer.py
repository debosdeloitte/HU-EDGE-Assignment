...
(spark.table("ecommerce.silver.event_items_question3")
    ...
    .saveAsTable("ecommerce.gold.monthly_revenue_by_product_city"))

# 2. Top 10 customers by lifetime value
(spark.table("ecommerce.bronze.sales_question3")
    .groupBy("email")
    .agg(F.round(F.sum("purchase_revenue_in_usd"), 2).alias("clv"),
         F.countDistinct("order_id").alias("orders"))
    .orderBy(F.desc("clv")).limit(10)
    .write.mode("overwrite").saveAsTable("ecommerce.gold.top10_customers_by_ltv"))

# 3. Conversion rate by traffic source
ev       = spark.table("ecommerce.bronze.events_question3")
...
(visitors.join(buyers, "traffic_source", "left")
    ...
    .write.mode("overwrite").saveAsTable("ecommerce.gold.conversion_by_traffic_source"))